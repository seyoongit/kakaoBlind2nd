...
<br>
<br>
각 엘리베이터마다 본인의 큐에 command를 넣어주고,  action 요청을 보낼때마다 이 큐에서 하나씩 빼다가 commands 를 만들어서 요청한다. 큐가 비면 각 엘리베이터 객체는 자신의 큐에 다시 command를 채워준다. 멀티쓰레딩을 공부할때 나오는 '생산자 공급자' 패턴과 모양이 같다.  
<br>
